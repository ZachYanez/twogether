...
export type SessionCondition = {
  /** `0` = full-session block (no periodic unlock windows). */
...
export type SavedSessionCondition = {
  ...
  label: string;
  defaultTitle: string;
  /** `0` = full-session block when used as a preset profile. */
  allowedMinutes: number;
  intervalHours: number;
  graceSeconds: number;
  description?: string;
...
export type User = {
  ...
  timezone: string;
  /** ISO timestamp: partner was active in the app recently (from `profiles.last_app_active_at`). */
