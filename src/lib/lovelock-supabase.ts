...
import {
  FULL_BLOCK_ALLOWED_MINUTES,
  FULL_BLOCK_INTERVAL_HOURS,
  fullBlockConditionFields,
} from '@/src/lib/session-block-mode';
import {
...
function normalizeProfileSnapshot(
  ...
  if (!value || typeof value !== 'object') {
    ...
    return {
      ...
      allowedMinutes: fallback.allowedMinutes ?? FULL_BLOCK_ALLOWED_MINUTES,
      intervalHours: fallback.intervalHours ?? FULL_BLOCK_INTERVAL_HOURS,
  ...
  return {
    ...
    defaultTitle,
    allowedMinutes:
      typeof value.allowedMinutes === 'number'
        ? value.allowedMinutes
        : fallback?.allowedMinutes ?? FULL_BLOCK_ALLOWED_MINUTES,
    intervalHours:
      typeof value.intervalHours === 'number'
        ? value.intervalHours
        : fallback?.intervalHours ?? FULL_BLOCK_INTERVAL_HOURS,
    graceSeconds:
...
function normalizeConditionSnapshot(value: JsonRecord | null | undefined): SessionCondition | null {
  ...
  return {
    ...
    essentialAppHints: Array.isArray(value.essentialAppHints)
      ? value.essentialAppHints.filter((entry): entry is string => typeof entry === 'string')
      : undefined,
    lockedAppLabels: Array.isArray(value.lockedAppLabels)
      ? value.lockedAppLabels.filter((entry): entry is string => typeof entry === 'string')
      : undefined,
    shortSessionDurationMinutes:
      typeof value.shortSessionDurationMinutes === 'number'
        ? value.shortSessionDurationMinutes
  ...
  const { error } = await supabase.from('profiles').upsert(
    {
      ...
      avatar_path: session.avatarUrl ?? null,
...
async function ensureDefaultTemplates(
  ...
  const { error: insertError } = await supabase.from('session_templates').insert(
    defaults.map((template) => ({
      ...
      profile_id: uuidOrNull(template.profileId),
    ...
    for (const occurrence of occurrences) {
      ...
      const { data: createdSession, error: sessionError } = await supabase
        ...
        .insert({
          ...
          template_id: uuidOrNull(template.id),
          ...
          condition_preset_id: uuidOrNull(template.profileId),
...
export async function pulseSelfProfileLastActive(session: AuthSession) {
  assertSupabaseEnabled();
  const supabase = getLovelockSupabaseClient();
  const now = new Date().toISOString();
  const { error } = await supabase
    .from('profiles')
    .update({ last_app_active_at: now })
    .eq('id', session.userId);

  assertNoError(error);
}

export async function fetchPartnerLastAppActive(partnerId: string): Promise<string | null> {
  assertSupabaseEnabled();
  const supabase = getLovelockSupabaseClient();
  const { data, error } = await supabase
    .from('profiles')
  ...
  assertNoError(error);
  return (data as { last_app_active_at?: string | null } | null)?.last_app_active_at ?? null;
}
...
export async function fetchRemoteSnapshot(
  ...
  const currentUser: User = {
    ...
    avatarUrl: profile.avatar_path ?? session.avatarUrl ?? null,
    timezone: profile.timezone || DEFAULT_TIMEZONE,
    lastAppActiveAt: profile.last_app_active_at ?? null,
  };
  ...
  const partner = firstPartnerProfile
    ? {
        ...
        avatarUrl: firstPartnerProfile.avatar_path ?? null,
        timezone: firstPartnerProfile.timezone || currentUser.timezone,
        lastAppActiveAt: firstPartnerProfile.last_app_active_at ?? null,
      }
    : currentCouple
...
export async function saveRemoteSessionPreset(session: AuthSession, draft: PresetDraft) {
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      allowed_minutes: block.allowedMinutes,
      interval_hours: block.intervalHours,
...
export async function createRemoteSessionTemplate(
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      profile_id: uuidOrNull(draft.profileId),
...
export async function createRemoteSession(
  ...
  const conditionSnapshot = draft.condition
    ? {
        ...
        lockedAppLabels: draft.condition.lockedAppLabels,
  ...
  const { data: createdSession, error: sessionError } = await supabase
    ...
    .insert({
      ...
      created_by_user_id: session.userId,
      template_id: uuidOrNull(draft.templateId),
      ...
      status,
      condition_preset_id: uuidOrNull(draft.condition?.presetId),
