...
import {
  activateNativeTimedShieldIfReady,
  armNativeTimedShieldIfArmed,
  deriveSessionStatusFromReadiness,
  tearDownNativeShieldForSession,
} from '@/src/lib/shield-session-native';
import {
  archiveRemotePlace,
  createRemotePairing,
  createRemoteSession,
  ...
  fetchRemoteSnapshot,
  fetchPartnerLastAppActive,
  hasSupabaseSync,
  joinRemotePairingByInviteCode,
  pulseSelfProfileLastActive,
...
} from '@/src/lib/lovelock-supabase';
import type {
  AuthSession,
  AuthStatus,
  AuthorizationStatus,
...
type LovelockState = {
  ...
  answerPairingPrompt: () => Promise<void>;
  requestPasswordReset: (email: string) => Promise<string>;
  updateAccountDisplayName: (displayName: string) => Promise<void>;
  updateAccountProfilePhoto: (avatarUrl: string | null) => Promise<void>;
  ...
  refreshPartnerPresence: () => Promise<void>;
  pulseSelfAppActivity: () => Promise<void>;
...
function makeInitialState() {
  return {
    ...
    selectionPreview: [
      { label: 'Apps', detail: 'Any apps selected in the Screen Time picker' },
      { label: 'Categories', detail: 'Games, entertainment, work, social, and more' },
      { label: 'Websites', detail: 'Specific domains selected in the picker' },
    ] as SelectionPreview[],
...
function canSyncRemoteSession(session: AuthSession | null | undefined): session is AuthSession {
  return Boolean(session && hasSupabaseSync() && isUuid(session.userId));
}
...
function bump(state: Omit<LovelockState, keyof ReturnType<typeof actionsFactory>>) {
  return {
    ...state,
...
function actionsFactory(
  set: (partial: Partial<LovelockState> | ((state: LovelockState) => Partial<LovelockState>)) => void
): LovelockActions {
  async function syncRemoteAppState(
    session: AuthSession,
    defaults?: SavedSessionCondition[]
  ) {
    if (!canSyncRemoteSession(session)) {
      return null;
    }
  ...
  async function syncLocationAutomationState(session: AuthSession | null) {
    const fallbackPermissionStatus = await getLocationPermissionStatus().catch(
      () => useLovelockStore.getState().locationPermissionStatus
    );
  ...
  async function maybeRunLocationAutomation() {
    const state = useLovelockStore.getState();
    ...
    const createdId = await useLovelockStore.getState().createSession({
  ...
  async function applyAuthenticatedSession(session: AuthSession) {
    const onboardingPreferences = await readStoredOnboardingPreferences();
    ...
    set((state) => ({
      ...
      onboarding: {
        ...state.onboarding,
        accountCreated: true,
        pairingPromptAnswered: onboardingPreferences.pairingPromptAnswered ?? false,
      },
...
function buildPreviewState(
  previewEmail = 'preview@lovelock.local',
  ...
  return {
    ...
    syncSubscriptionState: async () => {
      const session = useLovelockStore.getState().authSession;
    ...
    signInWithEmailPassword: async ({ email, password, displayName, avatarUrl }) => {
    ...
    registerWithEmailPassword: async ({ email, password, displayName, avatarUrl }) => {
    ...
    answerPairingPrompt: async () => {
      ...
      await writeStoredOnboardingPreferences({
        pairingPromptAnswered: true,
      });
    },
    ...
    updateAccountDisplayName: async (displayName) => {
      const existingSession = useLovelockStore.getState().authSession;
    ...
    updateAccountProfilePhoto: async (avatarUrl) => {
      const existingSession = useLovelockStore.getState().authSession;
      ...
      set((state) => ({
        ...
        currentUser: hasSupabaseSync()
          ? state.currentUser
            ? {
                ...state.currentUser,
                avatarUrl: response.session.avatarUrl ?? null,
              }
            : userFromSession(response.session)
          : userFromSession(response.session),
      }));
    ...
    deleteAccount: async () => {
      ...
      const savedSessionConditions = hasSupabaseSync()
        ...
        : useLovelockStore.getState().savedSessionConditions;
    ...
    pairWithPartner: async (partnerName = 'Riley') => {
      const session = useLovelockStore.getState().authSession;
      const inviteCode = useLovelockStore.getState().onboarding.inviteCode;
    ...
    savePlaceFromCurrentLocation: async ({ label, radiusMeters }) => {
      ...
      const permissionStatus =
        state.locationPermissionStatus === 'granted'
          ? 'granted'
          : await useLovelockStore.getState().requestLocationPermission();
      ...
      try {
        ...
        if (canSyncRemoteSession(session)) {
          ...
          const nextPlaces = [nextPlace, ...useLovelockStore.getState().savedPlaces];
    ...
    deleteSavedPlace: async (placeId) => {
      ...
      try {
        if (canSyncRemoteSession(session)) {
          ...
          const currentState = useLovelockStore.getState();
    ...
    refreshLocationAutomation: async () => {
      ...
      try {
        const coordinates = await getCurrentCoordinates();
        const places = useLovelockStore.getState().savedPlaces;
    ...
    refreshPartnerPresence: async () => {
      const session = useLovelockStore.getState().authSession;
      const partner = useLovelockStore.getState().partner;
      if (!session || !partner || partner.id.startsWith('pending-')) {
        return;
      }
      if (!canSyncRemoteSession(session) || !isUuid(partner.id)) {
        return;
      }
      try {
        const lastAppActiveAt = await fetchPartnerLastAppActive(partner.id);
        set((state) => ({
          ...bump(state),
          partner: state.partner ? { ...state.partner, lastAppActiveAt } : null,
        }));
      } catch {
        /* ignore transient presence fetch errors */
      }
    },
    pulseSelfAppActivity: async () => {
      const session = useLovelockStore.getState().authSession;
      if (!canSyncRemoteSession(session)) {
        return;
      }
      try {
        await pulseSelfProfileLastActive(session);
      } catch {
        /* ignore transient pulse errors */
      }
    },
    ...
    saveSessionCondition: async (draft) => {
      ...
      const block = fullBlockConditionFields();

      const session = useLovelockStore.getState().authSession;
      if (canSyncRemoteSession(session)) {
        const createdId = await saveRemoteSessionPreset(session, {
          ...draft,
          allowedMinutes: block.allowedMinutes,
          intervalHours: block.intervalHours,
        });
        await syncRemoteAppState(session);
        return createdId;
      }
    ...
    startTemplateSession: async (templateId) => {
      ...
      return useLovelockStore.getState().createSession({
    ...
    startQuickSession: async (conditionId) => {
      ...
      const durationMinutes = Math.max(
        20,
        condition.shortSessionDurationMinutes ?? 25
      );
      ...
      if (canSyncRemoteSession(session)) {
        ...
        await syncRemoteAppState(session);
        const quickStatus = deriveSessionStatusFromReadiness({
          scope,
          authorizationStatus: state.authorizationStatus,
          selectionConfigured: state.selectionConfigured,
        });
        await armNativeTimedShieldIfArmed({
          sessionStatus: quickStatus,
          sessionId: createdId,
    ...
    createSession: async (draft) => {
      ...
      if (canSyncRemoteSession(session)) {
        ...
        await syncRemoteAppState(session);
        const createdStatus = deriveSessionStatusFromReadiness({
          scope: draft.scope,
          authorizationStatus: state.authorizationStatus,
          selectionConfigured: state.selectionConfigured,
        });
    ...
    acceptSession: async (sessionId) => {
      ...
      if (canSyncRemoteSession(session)) {
        ...
        await syncRemoteAppState(session);
        if (nextStatus === 'armed') {
          const updated = useLovelockStore.getState().sessions.find((entry) => entry.id === sessionId);
          if (updated) {
            await armNativeTimedShieldIfArmed({
              sessionStatus: 'armed',
    ...
    activateSession: async (sessionId) => {
      ...
      if (new Date(targetSession.scheduledEndAt).getTime() <= Date.now()) {
        await useLovelockStore.getState().completeSession(sessionId);
        return;
      }
      ...
      if (canSyncRemoteSession(session)) {
        ...
        await activateNativeTimedShieldIfReady({
          sessionId,
          scheduledEndAt: targetSession.scheduledEndAt,
          authorizationStatus: state.authorizationStatus,
          selectionConfigured: state.selectionConfigured,
        });
        return;
    ...
    completeSession: async (sessionId) => {
      ...
      if (canSyncRemoteSession(session)) {
        ...
        await syncRemoteAppState(session);
        await tearDownNativeShieldForSession(sessionId);
        set((currentState) => ({
          ...bump(currentState),
          localShieldState: 'completed',
...
export const useLovelockStore = create<LovelockState>()((set) => ({
  ...makeInitialState(),
  ...actionsFactory(set),
}));
